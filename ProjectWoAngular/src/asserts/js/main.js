...
let activities = []
let activityNo = 0
let activityNoForInstructorChange = 0
...
function generatePOC() {
  ...
  ul.innerHTML = ""
  clients.forEach(e => {
    ul.innerHTML += `<li class="list-group-item" onclick="editClientFunc(${
      e.ClientNo
    })">${e.clientDetails()}</li>`
  })
  ...
  activities.forEach(e => {
    ul2.innerHTML += `<li class="list-group-item" onclick="showAddClientToActivity(${
      e.ActivityNo
    })">${e.ActivityDetails()}  </li>`
  ...
  activities[activity1.ActivityNo].ClientsList.forEach(cl => {
    ul_added.innerHTML += `<li class="list-group-item" onclick="removeClientFromActivity(${
      cl.ClientNo
    }, ${activity1.ActivityNo})">${cl.clientDetails()}</li>`
  })

  let ul3 = getElementById("instructors-group")
  ul3.innerHTML = ""
  instructors.forEach(e => {
    ul3.innerHTML += `<li class="list-group-item" onclick="editInstructorFunc(${
      e.InstructorNo
    })">${e.instructorDetails()}</li>`
  })
...
function SaveModified(ModifyClientNo) {
  ...
  if (
    ClientActiveCheck != getElementById("InputActive").checked &&
    getElementById("InputActive").checked == false
  ) {
    ...
    if (e.ClientNo == ClientNo) {
      ...
      getElementsByName("SEX").forEach(element => {
        ...
        if (
          ...
          element.type === "radio" &&
          element.value === loadedClient.Sex
        ) {
          element.checked
        }
      })
...
function InstructorBack() {
  ...
  getElementsByName("InstructorSEX").forEach(element => {
    // Sprawdź, czy element jest inputem typu radio
    if (
      element instanceof HTMLInputElement &&
      element.type === "radio" &&
...
function CreateNewActivity() {
  ...
  instructors.forEach(instructor => {
    if (instructor.active == true) {
      selectInst.innerHTML += `<option value='${
...
function saveToActivities() {
  ...
  ul.innerHTML = ""
  activities.forEach(e => {
    ul.innerHTML += `<li class="list-group-item" onclick="showAddClientToActivity(${
      e.ActivityNo
    })">${e.ActivityDetails()}  </li>`
    // <button onclick="showAddClientToActivity(${e.ActivityNo})">Add clients</button>
  })
...
function showAddClientToActivity(ActivityNo) {
  ...
  getElementById(
    "SignedClients"
  ).innerHTML = `Signed clients:  ${actv.SignedClients} / ${actv.MaxClients}`
  ...
  clients.forEach(e => {
    if (
      e.active == true &&
      activities[ActivityNo].ClientsList.indexOf(e) == -1
    ) {
      ul.innerHTML += `<li class="list-group-item" onclick="addClientToActivity(${
        e.ClientNo
      }, ${ActivityNo})">${e.clientDetails()}</li>`
    }
  })
  ...
  activities[ActivityNo].ClientsList.forEach(cl => {
    ul_added.innerHTML += `<li class="list-group-item" onclick="removeClientFromActivity(${
      cl.ClientNo
    }, ${ActivityNo})">${cl.clientDetails()}</li>`
  })

  getElementById(
    "client-add-to-activity-instructor"
  ).innerHTML = `Instructor: ${instructors[activities[ActivityNo].InstructorNo]
    .name +
    " " +
    instructors[activities[ActivityNo].InstructorNo].surname}`
...
function SaveInstructor() {
  ...
  newInstructor.Sex = querySelectorAll(
    'input[name="InstructorSEX"]:checked'
  ).value
  ...
  ul.innerHTML = ""
  instructors.forEach(e => {
    ul.innerHTML += `<li class="list-group-item" onclick="editInstructorFunc(${
      e.InstructorNo
    })">${e.instructorDetails()}</li>`
  })
  InstructorNo += 1
...
function editInstructorFunc(InstructorNo) {
  getElementById("instructor_create_form").style.display = "block"
  getElementById("instructor_list").style.display = "none"
  ...
  instructors.forEach(e => {
    if (e.InstructorNo == InstructorNo) {
      ...
      getElementById("InstructorInputHouseNumber").value =
        loadedInstructor.HouseNumber
      getElementById("InstructorInputFlatNumber").value =
        loadedInstructor.FlatNumber
      ...
      getElementsByName("InstructorSEX").forEach(element => {
        ...
        if (
          element instanceof HTMLInputElement &&
          element.type === "radio" &&
          element.value === loadedInstructor.Sex
        ) {
          element.checked
        }
      })
...
function SaveModifiedInstructor(ModifyInstructorNo) {
  ...
  modifiedInstructor.HouseNumber = getElementById(
    "InstructorInputHouseNumber"
  ).value
  modifiedInstructor.FlatNumber = getElementById(
    "InstructorInputFlatNumber"
  ).value
  ...
  if (
    InstructorActiveCheck != getElementById("InstructorInputActive").checked &&
    getElementById("InstructorInputActive").checked == false
  ) {
